"""
Launcher for the real eager-tier top-N precompute run. Reuses
run_precompute_batch unmodified (generation, grounding-check, section-
split, idempotency) -- only this file's job is to rank real candidates and
build their real context, per the corrected formula in
docs/decisions/phase3-eager-tier-precompute.md:

    N = floor(refresh_time_budget_seconds / real_measured_seconds_per_pair)

No phase or rank-bracket multiplier: one real generation call already
covers all three phases (confirmed throughout this project), and the real
match data only ever supports one real rank bracket ("emerald").
"""
import json
import time
...
PATCH = "16.15.1"
REFRESH_BUDGET_SECONDS = 28800  # 8 hours
MEASURED_SECONDS_PER_PAIR = 263.87  # real, fresh 5-pair timing sample this session
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"EAGER_TIER_TOPN_DONE wall_clock_s={t1 - t0:.1f} written={len(result['written_pairs'])} "
